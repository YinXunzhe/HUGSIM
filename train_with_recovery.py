...
def training(cfg):
    ...
    # Setup training and load checkpoint if exists
    ...
    first_iter = load_checkpoint(cfg.model_path, scene.gaussians, scene.dynamic_gaussians, unicycles if cfg.unicycle else {})
    ...
    for iteration in range(first_iter, cfg.train.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if iteration < cfg.train.iterations:
                ...
                # Save checkpoint periodically (every 1000 iterations)
                if iteration % 1000 == 0 and iteration not in cfg.train.checkpoint_iterations:
                    os.makedirs(scene.model_path + '/ckpts', exist_ok=True)
                    torch.save((gaussians.capture(), iteration), os.path.join(scene.model_path, f"ckpts/chkpnt{str(iteration)}.pth"))
                    for iid, dynamic_gaussian in scene.dynamic_gaussians.items():
                        torch.save((dynamic_gaussian.capture(), iteration), os.path.join(scene.model_path, f"ckpts/dynamic_{iid}_chkpnt{iteration}.pth"))
...
def load_checkpoint(model_path, gaussians, dynamic_gaussians, unicycles):
    ...
    ckpts = [f for f in os.listdir(ckpt_dir) if f.startswith("chkpnt") and f.endswith(".pth")]
    if not ckpts:
        return 0
    
    # Sort by iteration number
    ckpts.sort(key=lambda x: int(x[6:-4]))
    latest_ckpt = ckpts[-1]
    iteration = int(latest_ckpt[6:-4])
    
    print(f"Loading checkpoint from iteration {iteration}")
    
    # Load main Gaussian model
    (model_params, _) = torch.load(os.path.join(ckpt_dir, latest_ckpt))
    gaussians.restore(model_params)
    
    # Load dynamic Gaussians
    for iid in dynamic_gaussians.keys():
        ckpt_file = os.path.join(ckpt_dir, f"dynamic_{iid}_chkpnt{iteration}.pth")
        if os.path.exists(ckpt_file):
            (params, _) = torch.load(ckpt_file)
            dynamic_gaussians[iid].restore(params)
    
    # Load unicycles if exists
    if unicycles:
        for track_id in unicycles.keys():
            ckpt_file = os.path.join(ckpt_dir, f"unicycle_{track_id}.pth")
            if os.path.exists(ckpt_file):
                params = torch.load(ckpt_file)
                unicycles[track_id]['model'].restore(params)
